...
class Think {
    ...
    static public function halt($error) {
        ...
        if (APP_DEBUG || IS_CLI) {
        	...
        	$directory = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . API_ERR_LOG;
            ...
            if (!is_array($error)) {
                ...
                if(KDEBUGOUTPUT){
                	if(!is_dir($directory)){
                		if(mkdir($directory)){                	
                			$fd = fopen( $fileName , "a");
                			$log = date("Y-m-d H:i:s",time()) . PHP_EOL . "error =>" . $e['trace'] . PHP_EOL;
                			fwrite($fd,$log);
                			fclose($fd);
                		}
                	}
                ...
                if(KDEBUGOUTPUT){
                	if(KDEBUGOUTPUT){
                		if(!is_dir($directory)){
                			if(mkdir($directory)){
                				$fd = fopen( $fileName , "a");
                				$log = date("Y-m-d H:i:s",time()) . PHP_EOL . "error =>" . $e['message'] . PHP_EOL . $e['file'] .PHP_EOL . $e['line'] . PHP_EOL . $e['trace'] . PHP_EOL;                				
                				fwrite($fd,$log);
                				fclose($fd);
                			}
                		}
