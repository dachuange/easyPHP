...
function GetRequest(url) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			$('#night_driving_milladd').val(data.night_driving_milladd);
...
function submenu() {
	...
	var night_driving_milladd = $('#night_driving_milladd').val();
	...
	if (first_mileage == '') {
		alert('2-6公里，每公里里程费不能为空');
		return false
	}
	if (second_mileage == '') {
		alert('6公里以上，每公里里程费不能为空');
		return false
	}
	if (duration == '') {
		alert('时长费不能为空');
	...
	$.ajax({
		...
		data: {
			...
			night_driving_milladd: night_driving_milladd,
