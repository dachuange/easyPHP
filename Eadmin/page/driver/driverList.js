...
layui.use(['form', 'layer', 'table', 'laytpl'], function() {
			...
			[
				...
				},
				{
					...
					templet: function(d) {
						if (d.available_pd == 'Y') {
							return '正常'
						} else if (d.available_pd == 'snap_prohibited') {
							return '临时禁封'
						} else if (d.available_pd == 'lasting_prohibited') {
							return '永久禁封'
						}
					}
				},
	...
	$(".search_btn").on("click", function() {
		...
		table.reload("driverListTable", {
			...
			where: {
				...
				state: $('#state option:selected').val(),
				reviewed: $('#reviewed option:selected').val()
			},
		})
		layer.close(index);    //返回数据关闭loading
		// 		} else {
	...
	$(".daoAll_btn").on("click", function() {
		...
		var reviewed = $('#reviewed option:selected').val()
		if (state == ''&& reviewed!='') {
			window.location.href = url +'/admin/Exportdata/driverlistexporde/reviewed/' + reviewed+'/address_id/' + address_id;
		}
		if(reviewed==''&& state!=''){
			window.location.href = url +'/admin/Exportdata/driverlistexporde/state/' + state+'/address_id/' + address_id;
		}if(state!='' && reviewed!=''){
			window.location.href = url +'/admin/Exportdata/driverlistexporde/state/' + state+'/reviewed/' + reviewed+'/address_id/' + address_id;
		}if(state=='' && reviewed==''){
			window.location.href = url +'/admin/Exportdata/driverlistexporde/address_id/' + address_id;
		}
	});
	//列表操作
