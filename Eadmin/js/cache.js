...
layui.use(['form', 'jquery', "layer"], function() {
	...
	if (window.sessionStorage.getItem('userFace') && $(".userAvatar").length > 0) {
		...
		$(".userAvatar").attr("src", $(".userAvatar").attr("src").split("images/")[0] + "images/" + window.sessionStorage.getItem(
			'userFace').split("images/")[1]);
	} else {
	...
	function showNotice() {
		layer.open({
			...
			btn: ['火速围观'],
			moveType: 1,
			content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">系统公告</p><p><span class="layui-red">e达生活1.0正式上线，恭喜恭喜！</span></p></div>',
	...
	function lockPage() {
		...
		layer.open({
			...
			content: '<div class="admin-header-lock" id="lock-box">' +
				...
				'<div class="admin-header-lock-name" id="lockUserName">'+account+'</div>' +
				'<div class="input_btn">' +
			...
			success: function() {
				//判断是否设置过头像，如果设置过则修改顶部、左侧和个人资料中的头像，否则使用默认头像
				if (window.sessionStorage.getItem('userFace') && $(".userAvatar").length > 0) {
					$(".userAvatar").attr("src", $(".userAvatar").attr("src").split("images/")[0] + "images/" + window.sessionStorage
						.getItem('userFace').split("images/")[1]);
				}
			}
		})
